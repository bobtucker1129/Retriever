...
| **Runner host env — or single line in `retriever.env` (not a workflow input)** | **`RETRIEVER_SMOKE_EXPECT_FETCH_ENABLED=true`** — set **on the Windows host** (system or account env visible to the runner) **or** add **`RETRIEVER_SMOKE_EXPECT_FETCH_ENABLED=true`** to **`D:\retriever-rebuild\env\retriever.env`** (loaded by **`deploy.ps1`** into the deploy process **before** **`healthcheck.ps1`** and **`smoke.ps1`**) **only** while **`RetrieverRebuild`** intentionally runs with **`FETCH_ENABLED=true`**, so **`healthcheck.ps1`** and **`smoke.ps1`** expect **`checks.fetch`** and **`checks.modelProvider`** = **`ok`**. Omit for **`FETCH_ENABLED=false`** (foundation). **`post-deploy-feedback.ps1 -RunSmoke`** uses the **same pilot rule**: process env wins; if absent, it reads **only this key** from **`retriever.env`**. Same pattern as optional **`RETRIEVER_SMOKE_CF_URL`**. |
...
| File | Purpose |
|---|---|
| **`deploy-feedback/feedback.json`** | Machine-readable: version, health, Fetch `GET /fetch` status, optional broker **`GET …/health`**, legacy **`:8000`** probe, **`RetrieverRebuild` / `Retriever`** service states, optional **`smoke-transcript.txt`** metadata (**`expectFetchPilot`** when **`-RunSmoke`**). Non-secret keys read from **`retriever.env`** are limited to **`BOONEOPS_BROKER_URL`** (broker probe only — no bearer/HMAC) and, for the optional second smoke pass, **`RETRIEVER_SMOKE_EXPECT_FETCH_ENABLED`** when process env did not already set **`true`**. |
